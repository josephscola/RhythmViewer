...
2023.07.15  finally found the syntax to read a buffer with audioContext
            however the actual cause of the sound bug was the python version 
            beeping criterion which relied too much (and unnecessarily) on
            the clock stability.
            can't figure out why the buffer does need to be created each time 
            the beep is played...
"""
...
#        beep.play()
    ...
    async def setupSample (self):
        ...
        self.sample = []
        for n in self.N:
            filePath = path2sounds + 'beep_' + str(n) + '.wav'
            self.sample.append (await self.getFile (filePath))
        return None
    
    async def setupSample_ (self):
        #https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API/Advanced_techniques#dial-up_%E2%80%94_loading_a_sound_sample
        path2sounds = './sounds/'
        filePath = path2sounds + 'beep_8.wav'
        self.sample = await self.getFile(filePath)   
        return None
    

    def playSample (self, n = 0):
    ...
    def beep (self, n):
        ...
        if self.j [n] != jn:
            ...
            if n == 0: self.memo.innerHTML = str(self.j [n])
        return None
